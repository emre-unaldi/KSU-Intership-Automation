...
export const updateInternship = async (req, res) => {
    ...
    if (!_id) {
        return res.json({
            status: 'fail',
            message: 'Staj kimliği bulunamadığı için güncellenemedi'
        })
    } else {
        await Internship.findByIdAndUpdate(
            ...
            .then((data) => {
                return res.json({
                    status: 'success',
                    message: 'Staj kaydı başarıyla güncellendi',
                    data
                })
            }).catch((error) => {
                return res.json({
                    status: 'fail',
                    message: 'Staj kaydı güncellenemedi',
                    error
                })
            })
    }
...
export const deleteInternship = async (req, res) => {
    ...
    if (!_id) {
        return res.json({
            status: 'fail',
            message: 'Staj kimliği bulunamadığı için staj silinemedi'
        })
    } else {
        await Internship.findByIdAndDelete(_id)
            .then((data) => {
                return res.json({
                    status: 'success',
                    message: 'Staj kaydı başarıyla silindi',
                    data
                })
            }).catch((error) => {
                return res.json({
                    status: 'fail',
                    message: 'Staj kaydı silinemedi',
                    error
                })
            })
    }
...
export const sendInternshipConfirmationMail = async (req, res) => {
    const {studentID, internship} = await req.body
            ...
            if (currentUserInternship[0] !== undefined) {
                await transporter
                    ...
                    .then((data) => {
                        ...
                        return res.json({
                            status: 'success',
                            message: `Staj başvuru maili ${currentUserInternship[0].companyName} şirketine başarıyla gönderildi`,
                            smtpVerify: verified,
                            data
                        })
                    })
...
// Şirket onay durumu kontrol endpointi
export const companyApprovalStatus = async (req, res) => {
...
// Danışman onay durumu kontrol endpointi
export const consultantApprovalStatus = async (req, res) => {
